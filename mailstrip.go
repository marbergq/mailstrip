...
var (
	...
	quoteHeaderRegexp = regexp.MustCompile("(?m)^:etorw.*nO$|" + "^.*" + reverseString("på") + ".*?" + reverseString("From") + "$|^.*\\d{4}\\s\\.(.*?)\\s\\d+\\s(.*?)\\sneD$|(?m)^.*" + reverseString("Från:") + "(\\s+|^$)$|(?m)^.*verks.*neD$|^.*[0-9]{4}\\s\\.\\w{2,4}\\s\\d{1,2}\\s.{3,4}$|^\\w{3,4}\\s\\d{1,2}\\s\\w{3,4}\\.\\s[0-9]{4}.*$|^>.*\\d{1,2}/\\d{1,2}/\\d{4}$|^(?m)^.*?[0-9]{4}\\s\\.\\w+\\s\\d\\s.*n\\.*$")
)
